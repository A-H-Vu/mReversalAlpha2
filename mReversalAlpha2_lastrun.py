...
expName = 'mReversalAlpha2'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Andrew\\Documents\\Work\\Projects\\m-reversal-alpha2\\mReversalAlpha2_lastrun.py',
...
trials1 = data.TrialHandler(nReps=30, method='sequential', 
...
# completed 30 repeats of 'trials1'
...
# set up handler to look after randomisation of conditions etc
trials2 = data.TrialHandler(nReps=90, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
...
for thisTrials2 in trials2:
    ...
    # check responses
    if trial2Skip.keys in ['', [], None]:  # No response was made
        trial2Skip.keys = None
    trials2.addData('trial2Skip.keys',trial2Skip.keys)
    if trial2Skip.keys != None:  # we had a response
        trials2.addData('trial2Skip.rt', trial2Skip.rt)
    trials2.addData('trial2Skip.started', trial2Skip.tStartRefresh)
    trials2.addData('trial2Skip.stopped', trial2Skip.tStopRefresh)
    # the Routine "trial2" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
    
# completed 90 repeats of 'trials2'
...
# check responses
if instr3Resp.keys in ['', [], None]:  # No response was made
    instr3Resp.keys = None
thisExp.addData('instr3Resp.keys',instr3Resp.keys)
if instr3Resp.keys != None:  # we had a response
    thisExp.addData('instr3Resp.rt', instr3Resp.rt)
thisExp.addData('instr3Resp.started', instr3Resp.tStartRefresh)
thisExp.addData('instr3Resp.stopped', instr3Resp.tStopRefresh)
thisExp.nextEntry()
...
# set up handler to look after randomisation of conditions etc
trials3 = data.TrialHandler(nReps=30, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
...
for thisTrials3 in trials3:
    ...
    # check responses
    if trial3Skip.keys in ['', [], None]:  # No response was made
        trial3Skip.keys = None
    trials3.addData('trial3Skip.keys',trial3Skip.keys)
    if trial3Skip.keys != None:  # we had a response
        trials3.addData('trial3Skip.rt', trial3Skip.rt)
    trials3.addData('trial3Skip.started', trial3Skip.tStartRefresh)
    trials3.addData('trial3Skip.stopped', trial3Skip.tStopRefresh)
    # the Routine "trial3" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
    
# completed 30 repeats of 'trials3'
